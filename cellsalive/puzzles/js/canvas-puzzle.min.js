function playAudio(sAudio) {
	...
	window.sounds[sAudio].play()

}
function playAudio2(sAudio) {
...
(function()
{
    ...
    var events = (
        "dblclick mouseover mouseout mousedown mouseup click touchstart " +
...
(function()
{
    ...
    {
        if (f1.rotation % 360 || f2.rotation % 360 || f2.hide ||
            f1.hide || f1.row != f2.row && f1.col != f2.col)
        {
            return
        }
        var diff_x = f1.tx - f2.tx,
            ...
            s = f1.size;
        if ((diff_col == -1 && diff_x < 0 && abs(diff_x + s) <
                10 || diff_col == 1 && diff_x >= 0 && abs(
                    diff_x - s) < 10) && (diff_y <= 10 &&
                diff_y >= -10))
        {
            return [f1.col > f2.col ? -abs(diff_x) + s : abs(
                diff_x) - s, f2.ty - f1.ty]
        }
        else if ((diff_row == -1 && diff_y < 0 && abs(diff_y +
                    s) < 10 || diff_row == 1 && diff_y >= 0 &&
                abs(diff_y - s) < 10) && (diff_x <= 10 &&
                diff_x >= -10))
        {
            return [f2.tx - f1.tx, f1.row > f2.row ? -abs(
                diff_y) + s : abs(diff_y) - s]
        }
    }
    var Piece = Cevent.Shape.extend(
        {
            ...
            {
                var ctx = Util.getContext(document.createElement(
                        "canvas")),
                    s = this.size + .5;
                ...
                this.draw_path(ctx);
                ctx.clip();
                ctx.drawImage(this.img, -this.tx - ox, -
                    this.ty - oy);
                if (this.stroke)
                {
                    ctx.strokeStyle = "#000";
            ...
            outside: function(ctx, s, cx, cy)
            {
                ...
                ctx.bezierCurveTo(cx + s * .5, cy, cx + s *
                    .4, cy + s * -.15, cx + s * .4,
                    cy + s * -.15);
                ctx.bezierCurveTo(cx + s * .3, cy + s * -
                    .3, cx + s * .5, cy + s * -.3, cx +
                    s * .5, cy + s * -.3);
                ctx.bezierCurveTo(cx + s * .7, cy + s * -
                    .3, cx + s * .6, cy + s * -.15,
                    cx + s * .6, cy + s * -.15);
                ctx.bezierCurveTo(cx + s * .5, cy, cx + s *
                    .65, cy, cx + s * .65, cy);
            ...
            inside: function(ctx, s, cx, cy)
            {
                ...
                ctx.bezierCurveTo(cx + s * .505, cy + .05,
                    cx + s * .405, cy + s * .155, cx +
                    s * .405, cy + s * .1505);
                ctx.bezierCurveTo(cx + s * .3, cy + s * .3,
                    cx + s * .5, cy + s * .3, cx + s *
                    .5, cy + s * .3);
                ctx.bezierCurveTo(cx + s * .7, cy + s * .29,
                    cx + s * .6, cy + s * .15, cx + s *
                    .6, cy + s * .15);
                ctx.bezierCurveTo(cx + s * .5, cy, cx + s *
                    .65, cy, cx + s * .65, cy);
        ...
        Group = Cevent.Shape.extend(
        {
            ...
            check: function(other)
            {
                ...
                if (other.type == "piece")
                {
                    for (i = 0; i < l; i++)
                    {
                        if (r = check_position(this.pieces[
                                i], other))
                        {
                            this.rmove(r[0], r[1]);
                            return true
            ...
            add: function()
            {
                this.pieces = this.pieces.concat.apply(
                    this.pieces, arguments)
					playAudio("click.mp3");
            }
        });
...
(function()
{
    ...
    sound.load();
    window.sounds['ding.mp3'] = sound
	
	
	window.sounds = new Object();
    var sound = new Audio('sound/click.mp3');
    sound.load();
    window.sounds['click.mp3'] = sound
	
            var eventBus = new EventEmitter,
                self = this;
            this.opts = opts = Util.extend(opts ||
            {}, default_opts);
            ...
            this.ui = new jigsaw.UI(eventBus, opts.defaultPieces ||
                40);
            this.tmp_img = document.createElement(
                "img");
            ...
            this.parts = opts.defaultPieces || 40;
            this.tmp_img.onload = function()
            {
                self.original = this;
                self.draw_image(this);
                Util.calcPieces(self.img, self.opts
                    .piecesNumberTmpl);
                self.render()
            };
            ...
            jigsaw_events(this.ce, eventBus, this.opts
                .rotatePieces);
				
				eventBus.on(jigsaw.Events.JIGSAW_COMPLETE,function(){
	
		var sound = new Howl({
		  urls: ['sound/ding.mp3', 'sound/ding.wav'],
		  autoplay: true,
		  onend: function() {
	...
	self.ui.stop_clock();if(opts.redirect){self.redirect()}else{self.ui.show_time()}});
            if (opts.shuffled)
            {
                eventBus.on(jigsaw.Events.RENDER_FINISH,
                    this.shuffle.bind(this))

            }
            eventBus.on(jigsaw.Events.PARTS_NUMBER_CHANGED,
                this.set_parts.bind(this));
            eventBus.on(jigsaw.Events.RENDER_REQUEST,
                this.render.bind(this));
            eventBus.on(jigsaw.Events.JIGSAW_SHUFFLE,
                this.shuffle.bind(this));
            eventBus.on(jigsaw.Events.JIGSAW_SET_IMAGE,
                this.set_image.bind(this));
            eventBus.on(jigsaw.Events.SHOW_EDGE,
                function()
                {
                    self.ce.find("#middle").attr(
                        "hide", true);
                    self.ce.find("#edge").attr(
                        "hide", false);
                    self.ce.redraw()
                });
            eventBus.on(jigsaw.Events.SHOW_MIDDLE,
                function()
                {
                    self.ce.find("#middle").attr(
                        "hide", false);
            ...
            eventBus.on(jigsaw.Events.SHOW_ALL,
                function()
                {
                    self.ce.find("*").attr("hide",
                        false);
                    self.ce.redraw()
                });
            ...
            {
                var cv1 = self.ce.cv;
                cv1.width = self.container.offsetWidth,
                    cv1.height = self.container.offsetHeight -
                    45;
                self.ce.calcCanvasPosition();
                self.ce.redraw();
        ...
        draw_image: function(img, w, h)
        {
            ...
            if (max_w > this.container.offsetWidth ||
                max_h > this.container.offsetHeight -
                50)
            {
                var ratio = Math.min(this.container.offsetWidth /
                    max_w, (this.container.offsetHeight -
                        50) / max_h);
                max_w *= ratio;
                max_h *= ratio
            }
            if (img.width > max_w || img.height >
                max_h)
            {
                var rate = Math.min(max_w / img.width,
                    max_h / img.height);
                width = ctx.canvas.width = ~~(img.width *
                    rate);
                height = ctx.canvas.height = ~~(img.height *
                    rate);
                ctx.drawImage(img, 0, 0, img.width,
                    img.height, 0, 0, width,
                    height)
            }
            else
            {
        ...
        shuffle: function()
        {
            ...
            var i, l = T.length,
                ...
                ratio,
                h = (this.container.offsetHeight - 50) *
                ratio,
                padx = ~~((width - w) / 2),
                pady = ~~((height - h) / 2);
            for (i = 0; i < l; i++)
            {
                F = T[i];
                F.tx = Util.randint(w) + F.tx % 1 +
                    padx;
                F.ty = Util.randint(h) + F.tx % 1 +
                    pady;
        ...
        render: function()
        {
            ...
            offset = ~~(this.container.offsetWidth /
                2 - size * cols / 2);
            this.clear();
            ...
            ox = ox >= 0 ? ox : 0;
            oy = oy >= 0 ? oy : 0;
            this.preview.style.marginTop = this.opts.offsetTop +
                "px";
            this.preview.width = size * cols;
            this.preview.height = size * rows;
            this.previewCtx.drawImage(this.img, ox,
                oy, size * cols, size * rows, 0,
            ...
            (function F()
            {
                ...
                {
                    this.__pieces = this.ce.get()
                        .slice(0);
                    this.ce.redraw();
                    this.eventBus.emit(jigsaw.Events
                        .RENDER_FINISH)
                }
            }).bind(this)()
        }
    ...
    function jigsaw_events(ce, eventBus, rotate)
    {
        ...
        ce.drag("*",
        {
            ...
            move: function(c, e)
            {
                ...
                {
                    piece = pieces[i];
                    newX = piece.x + piece.tx +
                        deltaX;
                    newY = piece.y + piece.ty +
                        deltaY;

                    if (newX < 0 && deltaX < 0 ||
                        newX + size > width &&
                        deltaX > 0) c.lastX = c.x;
                    if (newY < 0 && deltaY < 0 ||
                        newY + size > height &&
                        deltaY > 0) c.lastY = c.y
                }
            },
            end: function(c, e)
            {
                ...
                for (; i < l; i++)
                {
                    ...
                    if (that.check(all[i]))
                    {
                        c.remove(that);
                        c.remove(all[i]);
                        c._curHover = c.group().get(-
                            1);
                        c._curHover.add(that.pieces ||
                            that, all[i].pieces ||
                            all[i]);
                        that = c._curHover;
                        c.focused = null
                ...
                if (!ce.getAll("piece").length &&
                    ce.getAll("group").length ==
                    1 && ce.shuffled)
                {
                    ce.shuffled = false;
        ...
        Util.addEvent(ce.cv, "dblclick", function(e)
        {
            if (rotate && ce.focused)
            {
			
			    
                ce.focused.rotation = (ce.focused.rotation +
                    45) % 360;
                ce.redraw()
            }
            e.preventDefault()
        ...
        ce.keydown("right", function()
        {
            if (this.focused)
            {
                this.focused.rotation = (this.focused
                    .rotation + 45) % 360
            }
            return false
        ...
        if (Cevent.isTouchDevice)
        {
            ce.click("*", function(c, e)
            {
                if (ce.focused)
                {
                    ce.focused.rotation = (ce.focused
                        .rotation + 45) % 360;
                    ce.redraw()
                }
            })
...
(function()
{
    ...
    jigsaw.UI = Class.extend(
    {
        init: function(eventBus, parts)
        {
            ...
            eventBus.on(jigsaw.Events.JIGSAW_SHUFFLE,
                this.init_clock.bind(this));
            eventBus.on(jigsaw.Events.SHOW_PREVIEW,
                this.show_preview.bind(this));
            eventBus.on(jigsaw.Events.SHOW_HELP, this
                .show_help.bind(this));
            eventBus.on(jigsaw.Events.SHOW_FILEPICKER,
        ...
        init_clock: function()
        {
            ...
            (function F()
            {
                if (self.uuid == uuid)
                {
                    ...
                    if (self.clock) self.clock.innerHTML =
                        self.time();
                    setTimeout(F, 1e3)
                }
            })()
        ...
        show_preview: function()
        {
            ...
            canvas.style.marginLeft = -(canvas.width /
                2) + "px"
        },
        show_time: function()
        {
            ...
            if (time) time.innerHTML = this.time();
            if (input) input.value = this.time()
        },
        time: function()
        {
            var t = ~~((this.last_time - this.ini) /
                    1e3),
                s = t % 60,
                m = ~~(t / 60),
                h = ~~(m / 60);
    ...
    function init_events(self, eventBus)
    {
        function handleFiles(files)
        {
            ...
            {
                eventBus.emit(jigsaw.Events.JIGSAW_SET_IMAGE,
                    this.result);
                close_lightbox()
            };
            reader.readAsDataURL(file)
        }
        if (false && window.FileReader && (new FileReader).onload ===
            null)
        {
            if ($("create"))
            {
            ...
            $("image-input") && Util.$("image-input").change(
                function()
                {
        ...
        $("set-parts") && Util.$("set-parts").change(function()
        {
            eventBus.emit(jigsaw.Events.PARTS_NUMBER_CHANGED, +
                this.value);
            eventBus.emit(jigsaw.Events.RENDER_REQUEST)
        });
        $("game-options") && Util.$("game-options")[Cevent.isTouchDevice ?
            "touchstart" : "click"]("a", function(e)
        {
